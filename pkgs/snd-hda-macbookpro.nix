# /etc/nixos/pkgs/snd-hda-macbookpro.nix
...
stdenv.mkDerivation rec {
  ...
  configurePhase = ''
    ...
    kernel_version="${linuxPackages.kernel.version}"
    ...
    tar -xf "${linuxPackages.kernel.src}" -C kernel-source --strip-components=1
...
EOF
    ...
    export KERNEL_BUILD="${linuxPackages.kernel.dev}/lib/modules/${linuxPackages.kernel.modDirVersion}/build"
    export KERNELRELEASE="${linuxPackages.kernel.modDirVersion}"
  ...
  installPhase = ''
    ...
    mkdir -p "$out/lib/modules/${linuxPackages.kernel.modDirVersion}/updates"
    ...
    if [ "$USE_CIRRUS" = "1" ]; then
      if [ -f "build/hda/snd-hda-codec-cirrus.ko" ]; then
        ...
        cp build/hda/snd-hda-codec-cirrus.ko "$out/lib/modules/${linuxPackages.kernel.modDirVersion}/updates/"
      else
      ...
      if [ -f "build/hda/snd-hda-codec-cs8409.ko" ]; then
        ...
        cp build/hda/snd-hda-codec-cs8409.ko "$out/lib/modules/${linuxPackages.kernel.modDirVersion}/updates/"
      else
    ...
    ls -la "$out/lib/modules/${linuxPackages.kernel.modDirVersion}/updates/"
