...
Rectangle {
    ...
    property var keyboardLayoutService: null
    ...
    Row {
        ...
        Repeater {
            ...
            WorkspaceIndicator {
                workspace: modelData
                workspaceWindows: {
                    if (!niriIPC?.windows || !modelData)
                        return [];
                    return niriIPC.windows.filter(window => window.workspace_id === modelData.id);
                }
    ...
    Row {
        ...
        function getColorForUsage(usage) {
            if (usage > 80)
                return Config.Material.colors.error;
            if (usage > 60)
                return Config.Material.colors.warning;
        ...
        KeyboardLayoutWidget {
            currentLayout: keyboardLayoutService?.currentLayout || "us"
            hasError: keyboardLayoutService?.hasErrors || false
            
            onLayoutSwitched: function(newLayout) {
                if (keyboardLayoutService) {
                    keyboardLayoutService.switchToLayout(newLayout)
                }
            }
        }
